...
public class product extends menu {
    ...
    public static void check_list(int mainNum, int productNum) {
        ...
        String price = "";
        String menuName = menu.name[mainNum - 1];
    ...
    public static void check(int mainNum, int productNum, int checkNum) {
        if (checkNum == 1) {
            if (mainNum == 2) {
                System.out.println("\n" + menu.name[mainNum - 1] + " | " + cakeData[productNum - 1][0] + " 이/가 장바구니에 추가되었습니다\n");
                order.add(menu.name[mainNum - 1], cakeData[productNum - 1][0], null, cakeData[productNum - 1][1]);
            } else if (mainNum == 3) {
                System.out.println("\n" + menu.name[mainNum - 1] + " | " + beverageData[productNum - 1][0] + " 이/가 장바구니에 추가되었습니다\n");
                order.add(menu.name[mainNum - 1], beverageData[productNum - 1][0], null, beverageData[productNum - 1][1]);
            } else if (mainNum == 4) {
                System.out.println("\n" + menu.name[mainNum - 1] + " | " + coffeeData[productNum - 1][0] + " 이/가 장바구니에 추가되었습니다\n");
                order.add(menu.name[mainNum - 1], coffeeData[productNum - 1][0], null, coffeeData[productNum - 1][1]);
            }
        } else if (checkNum == 2) {
    ...
    public static void iceCream_check(int mainNum, int sizeNum, int flavorNum, int checkNum) {
        if (checkNum == 1) {
            System.out.println(menu.name[mainNum - 1] + " | " + sizeData[sizeNum - 1][0] + " | " + flavorData[flavorNum - 1][0] + " 이/가 장바구니에 추가되었습니다\n");
            order.add(menu.name[mainNum - 1], sizeData[sizeNum - 1][0], flavorData[flavorNum - 1][0], sizeData[sizeNum - 1][1]);
        } else if (checkNum == 2) {
            System.out.println("취소하셨습니다.");
        } else {
            System.out.println("잘못된 접근입니다. ");
